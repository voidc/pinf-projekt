...
@Controller @RequestMapping("/search") public class SearchController {
	private GWERepository userRepository;

	@Autowired public SearchController(GWERepository userRepository) {
		this.userRepository = userRepository;
	}

	@RequestMapping(method = RequestMethod.GET) public ModelAndView search(
			@RequestParam(value = "q", required = false) String q,
			@RequestParam(value = "year", required = false) String year,
			@RequestParam(value = "disc", required = false) String disc,
			@RequestParam(value = "sort", required = false) String sort) {
		ModelAndView mav = new ModelAndView("search");
		Sort s = new Sort("lastName", "firstName", "graduationYear", "graduationType", "occupation", "disciplines",
				"id");

		if (sort != null) {
			switch (sort) {
			case "year":
				s = new Sort("graduationYear", "graduationType", "lastName", "firstName", "occupation", "disciplines",
						"id");
				mav.addObject("sortText", "Abschlussjahr");
				break;
			case "occu":
				s = new Sort("occupation", "lastName", "firstName", "graduationYear", "graduationType", "disciplines",
						"id");
				mav.addObject("sortText", "Beschäftigung");
				break;
			case "name":
				s = new Sort("lastName", "firstName", "graduationYear", "graduationType", "occupation", "disciplines",
						"id");
				mav.addObject("sortText", "Name");
				break;
			case "disc":
				s = new Sort("disciplines", "lastName", "firstName", "graduationYear", "graduationType", "occupation",
						"id");
	...
	private boolean testQuery(GWEUser user, String query) {
		...
		return user.getFirstName().toLowerCase().contains(query.toLowerCase()) || user.getLastName().toLowerCase()
				.contains(query.toLowerCase()) || user.getOccupation().toLowerCase().contains(query.toLowerCase());
	}
